...
def build_products_df(rakuten_df):
	...
	def get_additional_image_urls(row):
		image_urls = []
	...
	final_df['product_external_labels'] = rakuten_df.apply(get_product_labels, axis=1)
	final_df['product_price'] = rakuten_df.apply(get_price, axis=1).astype(float)
	final_df['product_sale_price'] = rakuten_df.apply(get_sale_price, axis=1).astype(float)
	...
	final_df['product_additional_image_urls'] = rakuten_df.apply(get_additional_image_urls, axis=1)
	return final_df
